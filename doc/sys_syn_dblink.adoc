...
* Install the `sys_syn` extension and its dependencies on the database that pulls the data, configure it, and add tables.
...
* Call `sys_syn_dblink.proc_table_create` for each table that you are bringing into this database.
...
- The `sys_syn` and `pgcrypto` PostgreSQL extensions installed and used on the database that you are connecting to.  These are not required for the database that needs sys_syn_dblink installed on it.
